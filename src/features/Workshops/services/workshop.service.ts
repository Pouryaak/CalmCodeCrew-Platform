...
export const getAllWorkshops = async (): Promise<Workshop[]> => {
  try {
    ...
    return workshopDocs.docs.map(
      (workshop) => ({ uid: workshop.id, ...workshop.data() }) as Workshop,
    );
  } catch (error) {
...
export const getOneWorkshop = async (
  workshopUid: string,
): Promise<Workshop> => {
  try {
...
export const updateWorkshop = async (
  ...
  try {
    ...
    return {
      uid: updatedWorkshopDoc.id,
      ...updatedWorkshopDoc.data(),
    } as Workshop;
  } catch (error) {
