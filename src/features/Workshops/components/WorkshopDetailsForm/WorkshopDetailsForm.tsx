...
const WorkshopDetailsForm: React.FC<WorkshopDetailsFormProps> = ({ id }) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      if (id) {
        try {
          const actionResult = await dispatch(modifyWorkshop(values));
          unwrapResult(actionResult);
        } catch (error) {
          console.error('Error updating workshop:', error);
        }
      } else {
        // If no ID is present, call the add new workshop function
        try {
          const actionResult = await dispatch(addNewWorkshop(values));
          unwrapResult(actionResult);
          resetForm();
  ...
  return (
    <div>
      ...
      />
      {storeState === STORE_STATUS.LOADING ? (
        <Loader />
      ) : (
        <form onSubmit={formik.handleSubmit}>
          ...
          <Box sx={{ m: 2 }}>
            <StudentsListField
              ...
              defaultSelectedStudentIds={formik.values.students}
            />
          </Box>
          <Box sx={{ m: 2 }}>
